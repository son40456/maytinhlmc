...
export async function generateMetadata({ params }: {
    ...
    if (data?.product) {
        try {
            ...
            return { title: 'Lỗi tải trang | LMC' };
        }
    }
    ...
    if (nodeData?.product) {
        try {
            ...
            const breadcrumbData = generateBreadcrumbSchema([
                ...
                { name: product.name || "Sản phẩm", url: `${SITE_URL}/${slug}` }
            ]);

            return (
                <div className="bg-[#f8fafc] min-h-screen pb-16">
                    <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(seoData.jsonLd).replace(/</g, '\\u003c') }} />
            ...
            return (
                <div className="min-h-screen flex items-center justify-center p-4">
                    <div className="bg-red-50 p-6 rounded-lg max-w-2xl w-full">
                        <h1 className="text-red-600 font-bold text-xl mb-4">⚠️ Lỗi render sản phẩm</h1>
                        <p className="text-red-500 mb-2">Hệ thống LMC đã bắt được lỗi này. Chi tiết:</p>
                        <pre className="text-xs text-slate-800 bg-white p-4 rounded overflow-auto border border-red-200">
                            {err.message + "\n" + (err.stack ? err.stack : "")}
                        </pre>
                    </div>
                </div>
