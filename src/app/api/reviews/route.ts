import { NextRequest, NextResponse } from 'next/server';

const WP_BASE = process.env.NEXT_PUBLIC_WORDPRESS_API_URL?.replace('/graphql', '') || 'https://apiserver.maytinhlmc.vn';
const WC_KEY = process.env.WC_CONSUMER_KEY || '';
const WC_SECRET = process.env.WC_CONSUMER_SECRET || '';

const isConfigured = () =>
    WC_KEY && WC_SECRET &&
    !WC_KEY.startsWith('ck_your') &&
    !WC_SECRET.startsWith('cs_your');

const wcAuth = () => 'Basic ' + Buffer.from(`${WC_KEY}:${WC_SECRET}`).toString('base64');

// ✅ Endpoint đúng của WooCommerce v3: /products/reviews (KHÔNG phải /products/{id}/reviews)
const REVIEWS_URL = `${WP_BASE}/wp-json/wc/v3/products/reviews`;

/** GET /api/reviews?productId=123 */
export async function GET(req: NextRequest) {
    const productId = req.nextUrl.searchParams.get('productId');
    if (!productId || !isConfigured()) return NextResponse.json({ reviews: [] });

    try {
        const url = `${REVIEWS_URL}?product=${productId}&per_page=50&status=approved`;
        const res = await fetch(url, {
            headers: { Authorization: wcAuth() },
            next: { revalidate: 60 },
        });
        if (!res.ok) return NextResponse.json({ reviews: [] });

        const data = await res.json();
        if (!Array.isArray(data)) return NextResponse.json({ reviews: [] });

        const reviews = data.map((r: any) => ({
            id: r.id,
            reviewer: r.reviewer,
            review: r.review?.replace(/<[^>]*>/g, '').trim() || '',
            rating: Number(r.rating) || 5,
            date: r.date_created,
            verified: r.verified ?? false,
        }));

        return NextResponse.json({ reviews });
    } catch {
        return NextResponse.json({ reviews: [] });
    }
}

/** POST /api/reviews  body: { productId, reviewer, reviewerEmail, review, rating } */
export async function POST(req: NextRequest) {
    if (!isConfigured()) {
        return NextResponse.json({ error: 'Chức năng đánh giá chưa được cấu hình.' }, { status: 503 });
    }
    try {
        const body = await req.json();
        const { productId, reviewer, reviewerEmail, review, rating } = body;

        if (!productId || !reviewer || !review || !rating) {
            return NextResponse.json({ error: 'Thiếu thông tin bắt buộc.' }, { status: 400 });
        }

        const res = await fetch(REVIEWS_URL, {
            method: 'POST',
            headers: {
                Authorization: wcAuth(),
                'Content-Type': 'application/json',
            },
            body: JSON.stringify({
                product_id: Number(productId),
                reviewer: reviewer.trim(),
                reviewer_email: reviewerEmail?.trim() || `${reviewer.toLowerCase().replace(/\s+/g, '.')}@guest.local`,
                review: review.trim(),
                rating: Number(rating),
            }),
        });

        const respJson = await res.json();

        if (!res.ok) {
            console.error('[reviews POST] WC error:', respJson);
            return NextResponse.json({ error: respJson.message || 'Gửi đánh giá thất bại.' }, { status: 400 });
        }

        return NextResponse.json({ success: true });
    } catch (err) {
        console.error('[reviews POST] exception:', err);
        return NextResponse.json({ error: 'Lỗi server, vui lòng thử lại.' }, { status: 500 });
    }
}
